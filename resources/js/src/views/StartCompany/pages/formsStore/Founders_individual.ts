...
export const foundersIdividualForm = defineStore("foundersIdividual", () => {
  ...
  const [identity_no_suffix] = defineField("identity_no_suffix");
  ...
  const identity_no_suffix_storage = useStorage("squareOne-fInd-identity_no_suffix", "");
  ...
  function updateFields(companyInProgress: any) {
    if (flat_storage.value.length > 1) flat.value = flat_storage.value;
    ...
    if (street2_storage.value.length > 1) street2.value = street2_storage.value;
    ...
    if (email_storage.value.length > 1) email.value = email_storage.value;
