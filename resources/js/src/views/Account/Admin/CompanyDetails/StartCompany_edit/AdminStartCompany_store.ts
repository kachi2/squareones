...
export const useStartCompanyStore = defineStore('startCompanyStore_edit', () => {
    ...
    // !!!!! leave this: '(company_id?: any)' this way because of other components already passing data
    const getCompanyInProgress = async (company_id?: any) => {
        try {
            const { data } = await api.adminCompanyProgress(currentCompanyId.value);
            // console.log(data, 'companyProgress');
