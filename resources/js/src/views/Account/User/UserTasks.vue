                        ...
                        <EasyDataTable class="easy-data-table"  :headers="itemsHeader" :items="items"
                            ...
                            <template #item-status="item">
                                <div class="status-dropdown">
                                        <div class="dropdown">
                                        <button :class="['btn', 'dropdown-toggle', 'btn-sm', 'btn-'+templateStore.taskStatusColor(item.status)]" type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false">
                                            <span class="status-indicator-inside bg-white me-2"></span>    {{item.status}}
                                        </button>
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <li><a class="dropdown-item status-todo" :style="{color: templateStore.taskStatusColor('TODO')} "  href="#" @click.prevent="updateTask(item, 'TO-DO')"><span class="status-indicator"></span> TO DO</a></li>
                                            <li><a class="dropdown-item status-doing" href="#"@click.prevent="updateTask(item, 'DOING')"><span class="status-indicator"></span> DOING</a></li>
                                            <li><a class="dropdown-item status-complete" href="#" @click.prevent="updateTask(item, 'COMPLETE')"><span class="status-indicator"></span> COMPLETE</a></li>
                                        </ul>
                                        </div>
                                    </div>
                            ...
                            <template #item-due_date="item">
                                <span v-if="checkDueDate(item.due_date)" :class="textColor"> {{ useFxn.dateDisplay(item.due_date) }} </span>
                            ...
                            <!-- <template #item-action="item">
                                <button @click="openUpdateTaskModal(item)" class="btn btn-sm btn-primary">
                                    View / Update
                                </button>
                            </template> -->
                        </EasyDataTable>
